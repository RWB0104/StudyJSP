...
function edit(editBtn)
{
	...
	// 글번호와 페이지 번호를 갖고 updateForm.jsp 페이지 로드
	var query = "updateForm.jsp?num=" + arr[0] + "&pageNum=" + arr[1];

	$("#main_board").load(query);
}

// [글삭제] 버튼을 클릭하면 자동 실행
// main.jsp의 main_board 영역에 글삭제 폼 표시
function del(delBtn)
{
	var rStr = delBtn.name;
	var arr = rStr.split(",");

	// 글번호와 페이지 번호를 갖고 deleteForm.jsp 페이지 로드
	var query = "deleteForm.jsp?num=" + arr[0] + "&pageNum=" + arr[1];
...
function reply(replyBtn)
{
	...
	var query = "writeForm.jsp?num=" + arr[0] + "&ref=" + arr[1] + "&re_step=" + arr[2] + "&re_level=" + arr[3] + "&pageNum=" + arr[4];
